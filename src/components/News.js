import React, {useEffect, useState} from 'react'
import NewsItem from './NewsItem.js'
import PropTypes from 'prop-types'


const News = (props) => {

  const [articles, setArticles] = useState([]);
  const [loading, setLoading] = useState(false);
  const [page, setPage] = useState(1);
  const [totalResults, setTotalResults] = useState(0);

  const updateNews = async() => {
    props.setProgress(0);
    let url = `https://newsapi.org/v2/top-headlines?country=${props.country}&pageSize=${props.pageSize}&page=${page}&category=${props.category}&apiKey=${props.ApiKey}`;
    props.setProgress(30);
    setLoading(true);
    let data = await fetch(url);
    props.setProgress(50);
    let parsedData = await data.json();
    setArticles(parsedData.articles);
    setTotalResults(parsedData.totalResults);
    setLoading(false);
    props.setProgress(100);
  } 
  
  useEffect(()=>{
    document.title = `LazyBee - ${props.category==="General" ? "Home" : props.category}`;
    updateNews();
    // eslint-disable-next-line
  }, []);
  
  const handlePrevClick = async () => {
    props.setProgress(0);
    let url = `https://newsapi.org/v2/top-headlines?country=${props.country}&pageSize=${props.pageSize}&page=${page-1}&category=${props.category}&apiKey=${props.ApiKey}`;
    props.setProgress(30);
    setLoading(true);
    let data = await fetch(url);
    props.setProgress(50);
    let parsedData = await data.json();
    setArticles(parsedData.articles);
    setTotalResults(parsedData.totalResults);
    setLoading(false);
    props.setProgress(100);
    setPage(page-1);
    // updateNews();
  }
  const handleNextClick = async () => {
    props.setProgress(0);
    let url = `https://newsapi.org/v2/top-headlines?country=${props.country}&pageSize=${props.pageSize}&page=${page+1}&category=${props.category}&apiKey=${props.ApiKey}`;
    props.setProgress(30);
    setLoading(true);
    let data = await fetch(url);
    props.setProgress(50);
    let parsedData = await data.json();
    setArticles(parsedData.articles);
    setTotalResults(parsedData.totalResults);
    setLoading(false);
    props.setProgress(100);
    setPage(page+1);
    }

  return (
    <>
      <div className="container my-3">
        <h1 className="text-center" style={{marginTop: "70px"}}><img style={{width: "45px", marginTop: "-7px", borderRadius: "50%"}}  src="data:image/jpeg;base64," alt="" />LazyBee Media - Top {props.category==="General" ? "" : props.category} Headlines</h1>
        {loading && <div className="container d-flex justify-content-center align-items-center" style={{height: "450px"}}>
        <span className="loader" style={{marginLeft: "-2rem"}}></span>
        </div>}
        {!loading && <div className="row">
        {articles.map((e)=>{
        return <div className="col md-4 my-3" key={e.url}>
          <NewsItem title={e.title? e.title:""} description={e.description? e.description:""} imageUrl={e.urlToImage? e.urlToImage : "https://images.albertsons-media.com/is/image/ABS/960309661?$ng-ecom-pdp-desktop$&defaultImage=Not_Available"} newsUrl={e.url} author={e.author} publishedAt={e.publishedAt} source={e.source} />
        </div>
        })}
        </div>}
        <div className="container d-flex justify-content-between my-2">
          <button type="button" className="btn btn-primary" onClick={handlePrevClick} disabled={page<=1}>&#8810; Previous</button>
          <button type="button" className="btn btn-primary" onClick={handleNextClick} disabled={page+1 > Math.ceil(totalResults/props.pageSize)} >Next &#8811;</button>
        </div>
      </div>
    </>
  )
}
News.defaultProps =   {
  country: "in",
  pageSize: 9,
  category: ""
}
News.PropsTypes =   {
  country: PropTypes.string,
  pageSize: PropTypes.number,
  category: PropTypes.string
}

export default News