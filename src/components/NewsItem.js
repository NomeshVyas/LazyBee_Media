...
const NewsItem = (props) => {
    ...
    return (
      <div className="card" style={{ width: "18rem", position: "relative" }}>
        <span className="badge rounded-pill " style={{position: "absolute", top: "-2%", right: "-4%", backgroundColor: "#C70039"}}>
          {source.name}
        </span>
        <img
          style={{ height: "12rem" }}
        ...
        <div className="card-body">
          ...
          <p className="card-text">
            <small className="text-body-secondary">
              {author ? "By" : ""} <b>{author}</b> {date ? "on" : ""} <span style={{color: "#333300", fontWeight: "500"}}>{date.getDate()} {month} {date.getUTCFullYear()}</span>
            </small>
          </p>
